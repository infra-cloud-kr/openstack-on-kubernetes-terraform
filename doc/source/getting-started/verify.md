...
K8s + OpenStack 컨트롤 플레인 + end-to-end VM 부팅까지 3계층으로 점검한다.
...
내부적으로 `aws ssm start-session --target <instance_id> --region ap-northeast-2`. 처음 한 번은 `Starting session with SessionId: ...` 메시지 뜨고 프롬프트가 `sh-5.1$`로 바뀐다. 이 프롬프트는 EC2 노드 **안**의 shell, default user는 `ssm-user` (sudoer).
...
- `session-manager-plugin not found` → 로컬에 플러그인 미설치, [설치 안내](install.md) 참조

### `KUBECONFIG` 환경변수가 왜 필요한가

`kubectl`은 클러스터 주소·인증서·토큰을 다음 우선순위로 찾는다:
1. `--kubeconfig` 플래그
2. **`$KUBECONFIG` 환경변수** ← 우리가 셋팅하는 것
3. `~/.kube/config`

`kubeadm init`이 만든 admin 자격증명은 `/etc/kubernetes/admin.conf`에 있는데 모드 0600·owner root라 일반 유저는 읽지 못한다. SSM 세션은 `ssm-user`로 시작하고, root로 승격해도 root의 `~/.kube/config`는 비어 있을 수 있어서 명시적으로 `export KUBECONFIG=...` 를 해준다.
...
`make status` 타겟은 옵션 B 방식(`sudo -u ubuntu kubectl ...`)을 쓴다.

## osc 파드로 OpenStack CLI 호출

OpenStack 명령어는 클러스터 안에 떠 있는 `osc` 파드(admin 크레덴셜이 envFrom으로 자동 주입)에서 실행한다. 한 번씩 호출:
...
`osc` 파드가 어떻게 admin 자격증명을 자동으로 받는지는 [자격증명 흐름](../architecture/overview.md)에서 설명한다.
...
# neutron 에이전트(L3 / DHCP / metadata / OVS) 모두 Alive=True, State=UP
kubectl -n openstack exec osc -- openstack network agent list
...
`make osh-vm`이 이미 (c)를 자동으로 수행한다. 따로 한 대 더 띄워서 확인하거나 ERROR 가 나는 경우는 [OpenStack 이용](../operations/using-openstack.md) / [트러블슈팅](../operations/troubleshooting.md) 참조.
