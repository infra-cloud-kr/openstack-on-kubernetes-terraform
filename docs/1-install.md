...
AWS EC2 한 대를 띄우고 그 위에 K8s + OpenStack-Helm 2024.2.0을 깔아 CirrOS VM 부팅까지 가는 전체 사이클.
...
**`make osh-deploy`**: 노드의 `osh/deploy.sh`가 SSM으로 실행됨 (16 step). 진행 중 SSM 명령이 InProgress → Success로 가는 동안 30초마다 status 폴링.
