...
설치/검증이 끝났으면 이제 직접 명령을 쳐보면서 익숙해질 차례.
...
`osc` 파드가 admin 자격증명을 어떻게 받는지는 [3.6](3-architecture.md#36-자격증명-흐름) 참조.
...
watch -n 2 openstack server list                 # BUILD → ACTIVE 로 바뀌는 거 (Ctrl-C로 종료)
```
...
- neutron 포트 생성 실패 → `openstack network agent list`에 :-(가 있는지

더 자세한 트러블슈팅 → [5장](5-troubleshooting.md).

---

다음 → [5. 트러블슈팅](5-troubleshooting.md)
